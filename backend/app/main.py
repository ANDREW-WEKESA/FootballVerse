...
def player_story(player_id: int, db: Session = Depends(get_db)):
    ...
    for stat in season_stats:
        events.append({
            ...
            "description": f"{stat.statistic}: {stat.value}",
            "team": stat.team,
            "league": stat.league,
    ...
    for honour in honours:
        events.append({
            ...
            "title": honour.honour,
            "description": (
                f"{honour.honour} with {honour.team}" if honour.team else honour.honour
            ),
            "team": honour.team,
    ...
    return {
        ...
        "intro": (
            f"Follow the football journey of {player.full_name}, from the early "
            "stages of their career through the major moments recorded in "
            "FootballVerse."
        ),
        "events": events,
...
def add_player_goal(player_id: int, goal: dict, db: Session = Depends(get_db)):
    ...
    if not player:
        raise HTTPException(status_code=404, detail="Player not found")
    goal_fields = [
        "date", "season", "team", "opponent", "competition", "minute",
        "score", "goal_type", "description", "video_url", "source_url",
        "evidence_type",
    ]
    new_goal = PlayerGoal(
        player_id=player_id,
        **{k: goal.get(k, "") for k in goal_fields},
...
def delete_player_goal(player_id: int, goal_id: int, db: Session = Depends(get_db)):
    goal = (
        db.query(PlayerGoal)
        .filter(PlayerGoal.id == goal_id, PlayerGoal.player_id == player_id)
        .first()
    )
    if not goal:
...
def update_goal_evidence(player_id: int, goal_id: int, evidence: dict, db: Session = Depends(get_db)):
    ...
    if not goal:
        raise HTTPException(status_code=404, detail="Goal not found")
    evidence_fields = [
        "video_url", "source_url", "evidence_type", "youtube_video_id",
        "youtube_timestamp", "youtube_channel", "youtube_title",
        "evidence_notes",
    ]
